...
class CheckResponseOfSurvey
{
    public static function checkAllResponseOfSurvey($user_id, $data)
    {
        ...
        if ($user_package['response_limit'] == 0 ||  ($user_package['response_limit'] - $sum_response_survey) > $data) {
            return true;
        }
        return false;
    }
    ...
    public static function checkDeleteLogo($user_id)
    {
        $time_now = Carbon::now();
        $user_package = UserPackage::getPackageUser($user_id, $time_now);
        if ($user_package['remove_ami_logo'] == 0) {
            return true;
        }
        return false;
    }
