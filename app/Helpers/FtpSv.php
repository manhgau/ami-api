...
class FtpSv
{
    public static function upload($file, $file_name, $ftpPath, $template_id)
    {
        try {
            $temp_path = 'uploads/survey/logo';
            ...
            if (ftp_put($ftp_conn, $ftp_put, $localPath, FTP_BINARY)) {
                ...
                ftp_close($ftp_conn);
                $data['logo'] =  '/' . $temp_path . '/' . $file_name;
                $update_logo = SurveyTemplate::updateSurveyTemplate($data, $template_id);
                if (!$update_logo) {
                    return false;
                }
                return  $data['logo'];
